...
export class SQLTRackingRepository implements ITrackingRepository {
  ...
  private async findTrackingCheckpoints(
    trackingIds: string[],
  ): Promise<Record<string, ITrackingCheckpoint[]>> {
    const checkpoints: ITrackingCheckpoint[] = await this.psqlConnection(
    ...
    return checkpoints.reduce(
      (map, checkpoint) => {
        map[checkpoint.tracking_internal_id] =
          map[checkpoint.tracking_internal_id] ?? [];
  ...
  async getTracking(
    filters: ITrackingFilterOptions,
    ...
    const checkPoints = await this.findTrackingCheckpoints(
  ...
  async save(
    groupedTrackings: IGroupedTracking[],
  ): Promise<IGroupedTracking[]> {
    const transactionClient = await this.psqlConnection.transaction();

    await transactionClient("trackings").insert(
      groupedTrackings.map((t) => t.tracking),
    );
