...
export class ParcelFashionCSVReader implements ICSVReader {
	...
	private groupCheckpointsByTracking(readCheckpoints: IParcelFashionCheckpoint[], groupedTracking: Record<string, ITracking>) {
		return readCheckpoints.reduce((map, detail) => {
	...
	private groupTracking(tracking: IParcelFashionTracking[]): Record<string, ITracking> {
		return tracking.reduce((map, item) => {
	...
	private convertTrackingCheckPoint(checkpoint: IParcelFashionCheckpoint, trackingInternalId: string): ITrackingCheckpoint {
		return {
